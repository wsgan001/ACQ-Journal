...
In practice, graphs are continuously envolving. For instance, users may change their profiles, make friends or remove friendship. Thus the CL-tree index needs to be updated to reflect the changes in the graph. However, rebuilding the CL-tree from scratch can be computationally expensive,  especially when the updates are very frequent. So we have developed efficient algorithms to maintain the CL-tree index for dynamic graphs.

In addition, we have proposed two typical variants of the ACQ problems which are called {\it Approximate ACQ problem} (or ACQ-A) and {\it Multiple-query problem} (or ACQ-M) respectively. ACQ-A is an approximation version of the ACQ query in which vertices of an AC do not need to exactly share the same keywords in $S$. It loosens the constraint on the number of shared keywords and find potential ACs if the keyword information is weak.
ACQ-M considers multiple query vertices and outputs their ACs. It can help to narrow the size of targeted ACs in which vertices are more relevant.
