...
In this section, we give the results of conducting experiments on both static and dynamic datasets. We first compare the performance between rebuilding the CL-tree and applying our algorithms for four static datasets in Figure~\ref{fig:exp-indexMaintenance}(a)-\ref{fig:exp-indexMaintenance}(h). We first separately conduct experiments in the edge insertion case and the edge deletion case. In two cases, we observe the performance of our algorithms with the core numbers of vertices increasing from 5 to 25. Then we randomly insert and delete 500 edges in four real datasets. 

We conduct experiments on two dynamic datasets (Flickr and Youtube~\cite{mislove-2009-socialnetworksthesis},~\cite{mislove-2008-flickr}) and attach the results in Figure~\ref{fig:exp-indexMaintenance}(i)-\ref{fig:exp-indexMaintenance}(j). Flickr dynamic dataset has $2,585,569$ vertices and $45,676,553$ edges, and Youtube dynamic dataset has $1,881,147$ vertices and $9,142,046$ edges. We record 100 daily snapshots of Flickr graph and Youtube graph are taken to record the evolving process of the orginal graphs. We divide these daily records to 5 periods and randomly select 200 records in each period to operate in the tree index. Note that Youtube dynamic dataset only records the edge insertion time~\cite{mislove-2009-socialnetworksthesis}. And for Flickr dynamic dataset, both the edge insertion and deletion records are included~\cite{mislove-2008-flickr}. 
