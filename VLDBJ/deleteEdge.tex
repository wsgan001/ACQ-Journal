...
\Function{moveUp($V^-$, $p$)}{}
    ...
    \EndIf
    \State update $p$;
    \If{ $ (p.father).coreNum$ = $c$--1}
       \State add $V^-$ to $p.father$;
    \Else
       \State create a new node $newFather$;
       \State add $V^-$ to $newFather$;
       \State add $newFather$ to the tree;
    \EndIf
    \State $set \gets$ $p.vertexSet$;
    \State $P \gets$ regroup vertices of $set$ and split node;
    \State update vertex-node map;
    \State link each $p_i \in P$ to $p.father$;
    \State update $invertedList$ of $p.father$;

    \State $P' \gets$ child nodes unconsidered in above step;
    \If{$P' \neq \emptyset$}
        re-link each $p \in P'$ to $p.father$;
    \EndIf
\EndFunction
\end{algorithmic}}
\end{algorithm}

We present the overall steps of {\tt deleteEdge} in Algorithm~\ref{alg:delete}.
...
\Function{deleteEdge($p_u,p_v$)}{}
    \If{$p_u.coreNum > p_v.coreNum$}
        ...
        \State $p_v' \gets$ split $p$ and its ancestor nodes;
        \State \Call{moveUp($V^-$, $p_v'$)}{};
    \ElsIf{$p_u.coreNum < p_v.coreNum$}
        \State compute $V^-$ in $p_u.vertexSet$;
        \State $p_u' \gets$ split $p$ and its ancestor nodes;
        \State \Call{moveUp($V^-$, $p_u'$)}{};
    \Else
        \State compute $V^-$ in $p_u.vertexSet$;
        \State $p_u',p_v' \gets$ split $p$ and its ancestor nodes;
        \If{$p_u' = p_v'$}
             \Call{moveUp($V^-$, $p_u'$)}{};
        \Else
             \State ${V_u}^-, {V_v}^- \gets$ separate $V^-$;
             \State \Call{moveUp(${V_u}^-$,$p_u'$)}{}; \textbf{ } \textbf{} \Call{moveUp(${V_v}^-$,$p_v'$)}{};
        \EndIf
