...
// o mark the root node
 ...
 //<item,users> 
 private Map<Integer, Set<Integer>> itemUsersMap=null;
 ...
 public FPTree(int k){
	 ...
	 this.itemUsersMap=new HashMap<Integer,Set<Integer>>();
...
public void addPrefixPath(List<FPNode> prefixPath, Map<Integer, Set<Integer>> mapSupportBeta, int relativeMinsup){
	// the first element of the prefix path contains the path support
//			int pathCount = prefixPath.get(0).getCount();  
			Set<Integer> pathUsers=prefixPath.get(0).getUser();
			...
			for(int i = prefixPath.size() -1; i >=1; i--){ 
				...
				if(mapSupportBeta.get(pathItem.getItem()).size() >= relativeMinsup){
					// check if there is a node already in the FP-Tree
					FPNode child = currentNode.hasChild(pathItem.getItem());
					if(child == null){ 
						// there is no node, we create a new one
						FPNode newNode = new FPNode(pathItem.getItem(),0);
						newNode.addUserSet(pathUsers);
 ...
 public void createHeaderList(final Map<Integer, Set<Integer>> mapSupport){
		...
		Collections.sort(headerList, new Comparator<Integer>(){
			public int compare(Integer id1, Integer id2){
				// compare the support
				int compare = mapSupport.get(id2).size() - mapSupport.get(id1).size();
