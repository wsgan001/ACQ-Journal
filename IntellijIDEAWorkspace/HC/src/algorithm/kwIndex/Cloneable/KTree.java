...
import algorithm.kwIndex.AUF.UnionFind;

/**
@author chenyankai
@Date	Aug 31, 2017
	cloneable KTree class 
	Using anti-monotonicity: build the K-trees dynamically
	Bottom up manner
	Using the k-tree in lower level to build the one in upper level with the Set<Integer> specialVertices to updates
*/
public class KTree implements Cloneable{
	...
	//the special vertices for dynamtic change the KTree
	
	private Set<Integer> specialVertice = null;  
	
	
	//clone the KTree class
	public KTree clone(){
		KTree object = null;
		try {
			object = (KTree) super.clone();
		} catch (CloneNotSupportedException e) {
			// TODO: handle exception
			System.out.println(e.toString());
		}
		
		object.vertexMap = new HashMap<Integer,KNode>(vertexMap.size());
		Iterator<Entry<Integer, KNode>> it = vertexMap.entrySet().iterator();
		while(it.hasNext()){
			Entry<Integer, KNode> entry = it.next();
			object.vertexMap.put(entry.getKey(), entry.getValue().clone());
		}
		return object;
	}
