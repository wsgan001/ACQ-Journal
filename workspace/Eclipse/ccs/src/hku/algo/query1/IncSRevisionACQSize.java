 ...
 * @date Aug 17, 2015
 */
public class IncSRevisionACQSize {
	...
	public IncSRevisionACQSize(int graph[][], String nodes[][], TNode root, int core[], String ccsFile){
		this.graph = graph;
		this.nodes = nodes;
		this.root = root;
		this.core = core;
		this.ccsFile = ccsFile;
		this.saver = new CCSSaver("/home/fangyixiang/Desktop/lac");
	}
...
//				System.out.println("], coreness:" + entry.getValue());
				...
				if(ccsSet.size() > 1){
					...
					validKwList.add(kws);
					validCoList.add(min);
//					System.out.println("queryId:" + queryId + " |lac| = " + ccsSet.size() + "   Time:" + (System.currentTimeMillis() - startT));
					if(iterK >= 1)   saver.save(graph, nodes, queryId, ccsSet, kws);
				}else{
...
//			System.out.println("validKwList.size:" + validKwList.size() + "   candMap.size:" + candMap.size() + "\n");
		...
		for(int i = 1;i <= 5;i ++){
			if(count[i] > 0){
				System.out.println("|S|:" + i + " Size:" + sum[i] * 1.0 / count[i]);
			}else{
				System.out.println("|S|:" + i + " No results");
			}
		}
...
//		System.out.println("cckMap.size:" + cckMap.size());
	...
	private Set<Integer> findCCS(String kws[], TNode root){
		...
		while(queue.size() > 0){
			...
			for(String keyword:kws)   kwsSet.add(keyword);
			Set<Integer> intersecSet = new HashSet<Integer>();
			for(int id:curNode.getNodeSet()){
				int tmpCount = 0;
				for(String word:nodes[id]){
					if(kwsSet.contains(word)){
						tmpCount += 1;
					}
				}
				if(tmpCount >= kwsSet.size())   intersecSet.add(id);
			}
