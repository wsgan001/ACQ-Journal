 ...
 * @author fangyixiang
 * @date Aug 17, 2015
 * This class is used to compute the values on these metrics to justify that we need *maximize*
 */
public class IncSNumber {
...
//	private CCSSaver saver = null;
	...
	public IncSNumber(int graph[][], String nodes[][], TNode root, int core[], String ccsFile){
		this.graph = graph;
		this.nodes = nodes;
		this.root = root;
		this.core = core;
	...
	public List<Map<Integer, Double>> query(int queryId, int fangLen){
//		System.out.println("IncS-queryId:" + queryId);
		...
		int maxLen = 0;
		Map<Integer, Double> cmfMap = new HashMap<Integer, Double>();//shareNum -> CMF value
		Map<Integer, Double> cwfMap = new HashMap<Integer, Double>();//shareNum -> CWF value
		Map<Integer, Double> cpjMap = new HashMap<Integer, Double>();//shareNum -> CPF value
		Map<Integer, Double> cqjMap = new HashMap<Integer, Double>();//shareNum -> CQF value
		List<Map<Integer, Double>> fangList = new ArrayList<Map<Integer, Double>>();
		fangList.add(cmfMap);
		fangList.add(cwfMap);
		fangList.add(cpjMap);
		fangList.add(cqjMap);
...
//					System.out.println("queryId:" + queryId + " No community Time:" + (System.currentTimeMillis() - startT));
			...
			double aqjValue = aqj.sim(ccsList, queryId);
			double apjValue = apj.sim(ccsList);
			double awfValue = awf.sim(ccsList, queryId);
			double adfValue = adf.sim(ccsList, queryId);
			
			cqjMap.put(iterK, aqjValue);
			cpjMap.put(iterK, apjValue);
			cwfMap.put(iterK, awfValue);
			cmfMap.put(iterK, adfValue);
			
			if(validKwList.size() == 0 || iterK >= fangLen){
				break;
			}else{
