...
public class KTree {
	...
	public KTree(int[][] subGraph){
		this.vertexList = subGraph[0];
		this.subGraph = subGraph;
		this.n = subGraph.length;
		...
		while(entryIter.hasNext()){
			...
			if(core[id] == curCoreNum){
				if(ufNodeMap.containsKey(newFather)){
					ufNodeMap.get(newFather).add(vertexList[id]);
				}else{
					Set<Integer> set = new HashSet<Integer>();
					set.add(vertexList[id]);
			...
			//childList
				...
				KNode kNode = vertexMap.get(vertexList[oldFather.anchor]);
