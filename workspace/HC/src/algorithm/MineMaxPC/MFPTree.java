...
//insert one possible MFP in the MFP-tree
public void insertFPNodes(List<FPNode> itemset,int support){
	 MFPNode currentNode = root;
	 for(int i=0;i<itemset.size();i++){
		 FPNode itemNode=itemset.get(i);
		 //check if there is a node already in the MFP-tree
		 MFPNode child = currentNode.hasChild(itemNode.getItem());
		 if(child == null ){
			 	MFPNode node =new MFPNode(itemNode.getItem(),i+1);
				node.linkFather(currentNode);
				currentNode.linkChild(node);
				UpdateNodeLinks(itemNode.getItem(), node);
				node.addUserSet(itemNode.getUser());
				currentNode=node;
		 }
		 else{
			 	child.addUserSet(itemNode.getUser());
				currentNode=child;
		 } 
	 } 
}
...
public boolean SubsetChecking(List<FPNode> headWithP) {

	// Find the node list for the last item of the itemset
	int lastItem = headWithP.get(headWithP.size()-1).getItem();
...
private boolean isASubsetOfPath(List<FPNode> headWithP, MFPNode node) {
	...
	if(node.getLevel() >= headWithP.size()) {
		...
		int itemToLookFor = headWithP.get(positionInHead).getItem();
		...
		do {
			if(nodeToCheck.getItem() == itemToLookFor) {
				...
				itemToLookFor = headWithP.get(positionInHead).getItem();
			}
