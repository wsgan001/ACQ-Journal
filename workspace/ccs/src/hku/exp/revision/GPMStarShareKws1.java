 ...
 * @date May 24, 2016
 */
public class GPMStarShareKws1 {
	private int queryId = -1;
	private int graph[][] = null;
	private String nodes[][] = null;
	
	public GPMStarShareKws1(int graph[][], String nodes[][]){
		this.graph = graph;
		this.nodes = nodes;
	}
...
//		set.add("network");
	...
	private Set<Integer> findHop3Keyword(Set<String> set){
		...
		for(int i = 1;i < graph[queryId].length;i ++){
			...
			if(contain(firstNgh, set)){
				...
				System.out.println("1-select:" + firstNgh + "[" + nodes[firstNgh][0] + "]");
			}
		}
		for(int i = 1;i < graph[queryId].length;i ++){
			int firstNgh = graph[queryId][i];
			if(!selectSet.contains(firstNgh)){
				for(int j = 1;j < graph[firstNgh].length;j ++){
					...
					if(contain(secondNgh, set) && !selectSet.contains(secondNgh)){
						...
						System.out.println("2-select:" + firstNgh + "[" + nodes[firstNgh][0] + "]" + " " 
	...
	public static void main(String[] args) {
		...
		GPMStarShareKws1 cmp = new GPMStarShareKws1(graph, nodes);
		cmp.comp(15238);
//		cmp.comp(152532);
	}
