	...
	public void exp(String graphFile, String nodeFile){
		...
		for(int i = 0;i < queryIdList.size();i ++){
			...
			if(ccsList != null && ccsList.size() > 0){
				...
				aqjSum1 += aqj.singleSim(sigkddSet, queryId);
				aqjSum2 += aqj.singleSim(sigmodSet, queryId);
				aqjSum3 += aqj.sim(ccsList, queryId);
				
				apjSum1 += apj.singleSim(sigkddSet);
				apjSum2 += apj.singleSim(sigmodSet);
				apjSum3 += apj.sim(ccsList);
				
				awfSum1 += awf.singleSim(sigkddSet, queryId);
				awfSum2 += awf.singleSim(sigmodSet, queryId);
				awfSum3 += awf.sim(ccsList, queryId);
		...
		Log.log("count:" + count);
		Log.log("AQJSim:" + aqjSum1 / count + " " + aqjSum2 / count + " " + aqjSum3 / count);
		Log.log("APJSim:" + apjSum1 / count + " " + apjSum2 / count + " " + apjSum3 / count);
		Log.log("AWFreq:" + awfSum1 / count + " " + awfSum2 / count + " " + awfSum3 / count);
